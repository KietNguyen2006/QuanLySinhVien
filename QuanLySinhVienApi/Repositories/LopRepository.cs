using Dapper;
using System.Data;
using Microsoft.Data.SqlClient;
using QuanLySinhVienApi.Models;

namespace QuanLySinhVienApi.Repositories
{
    public class LopRepository
    {
        private readonly string? _connectionString;

        public LopRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DefaultConnection");
        }

        private IDbConnection Connection => new SqlConnection(_connectionString);

        public IEnumerable<Class> GetAll()
        {
            using (var db = Connection)
            {
                return db.Query<Class>("SELECT * FROM Lop");
            }
        }

        public Class? GetById(int id)
        {
            using (var db = Connection)
            {
                return db.QueryFirstOrDefault<Class>("SELECT * FROM Lop WHERE LopId = @Id", new { Id = id });
            }
        }

        public int Create(Class lop)
        {
            using (var db = Connection)
            {
                var sql = "INSERT INTO Lop (ClassName, AcademicYear) VALUES (@ClassName, @AcademicYear)";
                return db.Execute(sql, lop);
            }
        }

        public int Update(Class lop)
        {
            using (var db = Connection)
            {
                var sql = "UPDATE Lop SET ClassName = @ClassName, AcademicYear = @AcademicYear WHERE LopId = @Id";
                return db.Execute(sql, lop);
            }
        }

        public int Delete(int id)
        {
            using (var db = Connection)
            {
                return db.Execute("DELETE FROM Lop WHERE LopId = @Id", new { Id = id });
            }
        }
    }
}