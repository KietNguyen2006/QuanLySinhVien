...
namespace QuanLySinhVienApi.Repositories
{
    public class MonHocRepository
    {
        ...
        public Subject? GetById(int id)
        {
            using (var db = Connection)
            {
                return db.QueryFirstOrDefault<Subject>("SELECT * FROM MonHoc WHERE MonHocId = @Id", new { Id = id });
            }
        }
        ...
        public int Update(Subject monHoc)
        {
            using (var db = Connection)
            {
                var sql = "UPDATE MonHoc SET SubjectName = @SubjectName, Credits = @Credits WHERE MonHocId = @Id";
                return db.Execute(sql, monHoc);
            }
        }
